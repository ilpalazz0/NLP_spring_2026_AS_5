...
def main() -> None:
    ...
    examples = [EvaluationExample(**row) for row in read_jsonl(eval_path)]
    if not examples:
    ...
    if vector_store.count() == 0:
        raise ValueError("Vector store is empty. Build the knowledge base first.")
    ...
    metrics["settings"] = {
        ...
        "ollama_model": settings.ollama_model if settings.generation_provider in {"ollama", "qwen"} else None,
        "embedding_model_name": settings.embedding_model_name,
        "top_k": args.top_k,
        "use_reranker": settings.use_reranker,
        "reranker_model_name": settings.reranker_model_name if settings.use_reranker else None,
    }
