...
def normalize_cli_title(text: str) -> str:
    return normalize_ws(text.replace("_", " ").replace("-", " "))


AZ_MATCH_TRANSLATION = str.maketrans({
    "ə": "e",
    "Ə": "e",
    "ğ": "g",
    "Ğ": "g",
...
def fold_for_match(text: str) -> str:
    folded = normalize_cli_title(text).translate(AZ_MATCH_TRANSLATION).casefold()
    folded = folded.replace("'", " ").replace("’", " ")
    return normalize_ws(folded)


def tokenize_for_match(text: str) -> List[str]:
    return [token for token in re.split(r"[^a-z0-9]+", fold_for_match(text)) if token]


def generate_search_queries(raw_title: str) -> List[str]:
    base = normalize_cli_title(raw_title)
    if not base:
        return []

    variants = [base]

    transliterated = base.casefold()
    transliterated = re.sub(r"aga", "ağa", transliterated)
    transliterated = re.sub(r"bey", "bəy", transliterated)
    transliterated = re.sub(r"oglu", "oğlu", transliterated)
    transliterated = transliterated.replace("sh", "ş")
    transliterated = transliterated.replace("ch", "ç")
    transliterated = transliterated.replace("kh", "x")
    transliterated = transliterated.replace("gh", "ğ")
    transliterated = transliterated.replace("zh", "j")
    transliterated = transliterated.replace("azerbaycan", "azərbaycan")
    transliterated = transliterated.replace("nesri", "nəsri")
    transliterated = transliterated.replace("antologiyasi", "antologiyası")
    transliterated = transliterated.replace("edebiyyat", "ədəbiyyat")
    transliterated = transliterated.replace("edebiyyati", "ədəbiyyatı")

    azeri_variant = " ".join(word.capitalize() for word in transliterated.split())
    if azeri_variant:
        variants.append(azeri_variant)

    lower_base = base.casefold()
    if lower_base == "classical literature":
        variants.extend(["Klassik ədəbiyyat", "Azərbaycan klassik ədəbiyyatı"])
    if lower_base == "azerbaijani drama":
        variants.extend(["Azərbaycan dramaturgiyası", "Azərbaycan dramı"])
    if " literature" in lower_base:
        variants.append(re.sub(r"literature", "ədəbiyyat", base, flags=re.IGNORECASE))
    if " drama" in lower_base:
        variants.append(re.sub(r"drama", "dramı", base, flags=re.IGNORECASE))

    deduped: List[str] = []
    seen: Set[str] = set()
    for variant in variants:
        cleaned = normalize_ws(variant)
...
class WikiScraper:
    ...
    def search_page_titles(self, query: str, limit: int = 10) -> List[str]:
        data = self.api_get(
            {
                "action": "query",
                "list": "search",
        ...
        for result in data.get("query", {}).get("search", []):
            title = normalize_ws(result.get("title", ""))
...
class IncrementalState:
    ...
    def _load_or_bootstrap_processed_pages(self) -> Set[str]:
        ...
        if recovered:
            with self.processed_pages_path.open("w", encoding="utf-8") as handle:
                for value in sorted(recovered):
                    handle.write(value + "\n")
        return recovered
...
def append_candidate(
    ...
    allow_already_processed: bool = False,
) -> bool:
    if len(scheduled) >= limit:
        return False
    if should_skip_title(title) or title in scheduled_set:
        return False
    if not allow_already_processed and title in processed_pages:
        return False
...
def score_target_candidate(raw_title: str, candidate_title: str, categories: List[str], html: str) -> int:
    raw_folded = fold_for_match(raw_title)
    candidate_folded = fold_for_match(candidate_title)
    raw_tokens = set(tokenize_for_match(raw_title))
    candidate_tokens = set(tokenize_for_match(candidate_title))
    overlap = raw_tokens & candidate_tokens

    score = 0
    if raw_folded == candidate_folded:
        score += 12

    score += 4 * len(overlap)
    if raw_tokens:
        overlap_ratio = len(overlap) / len(raw_tokens)
    ...
    if is_probable_person_name(raw_title) and is_probable_person_name(candidate_title):
        score += 2

    category_blob = " | ".join(category.casefold() for category in categories)
    score += min(count_keyword_hits(category_blob, LITERATURE_CATEGORY_KEYWORDS), 3) * 2

    if contains_keyword(candidate_title.casefold(), LITERATURE_TITLE_KEYWORDS):
        score += 2

    if html and is_literature_page(candidate_title, categories, html):
        score += 6
    elif html:
        score -= 8
...
def resolve_target_title(scraper: WikiScraper, raw_title: str) -> Optional[str]:
    queries = generate_search_queries(raw_title)

    # First try the provided title directly with redirects enabled.
    # This is safer than search because MediaWiki handles many spelling variants itself.
    for query in queries:
        try:
            parsed = scraper.get_parse(query)
        except Exception:
            continue

        resolved_title = normalize_ws(parsed.get("title") or query)
        html = parsed.get("text", "")
        categories = extract_categories(parsed)
        if html and is_literature_page(resolved_title, categories, html):
            return resolved_title

    best_title: Optional[str] = None
    best_score = -10**9

    # Search is only a fallback, and we score candidates before accepting any mapping.
    for query in queries:
        for candidate in scraper.search_page_titles(query, limit=8):
            if should_skip_title(candidate):
                continue
            try:
                parsed = scraper.get_parse(candidate)
            except Exception:
                continue

            resolved_title = normalize_ws(parsed.get("title") or candidate)
            html = parsed.get("text", "")
            categories = extract_categories(parsed)
            score = score_target_candidate(raw_title, resolved_title, categories, html)

            if score > best_score:
                best_score = score
                best_title = resolved_title

    # Reject weak mappings instead of sending the crawl to unrelated pages.
    if best_title is None:
        return None

    raw_tokens = set(tokenize_for_match(raw_title))
    best_tokens = set(tokenize_for_match(best_title))
    overlap = raw_tokens & best_tokens

    if best_score >= 10 and overlap:
        return best_title
    if raw_tokens and len(overlap) >= max(1, min(2, len(raw_tokens))):
        return best_title

    return None


def discover_from_target_pages(
    scraper: WikiScraper,
    ...
    if not target_pages or len(scheduled) >= max_new_pages:
        return

    print("[INFO] Scheduling requested target pages...")
    added = 0
    for raw_title in target_pages:
        if len(scheduled) >= max_new_pages:
            break
        resolved_title = resolve_target_title(scraper, raw_title)
        chosen_title = resolved_title or normalize_cli_title(raw_title)
        if not chosen_title:
            continue
        if append_candidate(
            chosen_title,
            ...
            processed_pages,
            max_new_pages,
            allow_already_processed=True,
        ):
            added += 1
            if chosen_title == normalize_cli_title(raw_title):
                print(f"[OK] Target scheduled: {chosen_title}")
            else:
                print(f"[OK] Target scheduled: {raw_title} -> {chosen_title}")
        else:
            print(f"[INFO] Target already scheduled or skipped: {chosen_title}")
    print(f"[INFO] Added {added} requested target pages")
...
def discover_from_seed_pages(
    ...
    for seed_title in seed_pages:
        ...
        try:
            ...
            new_from_seed = 0
            for linked_title in extract_internal_links_from_html(html):
                if append_candidate(
                    linked_title,
                    scheduled,
                    scheduled_set,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Add a specific author or page title to search for. Repeat the flag to add multiple targets.",
    )
    parser.add_argument(
        "--target_pages_file",
        type=str,
        default=None,
        help="Plain text file with one author/page title per line.",
    )
...
def load_target_pages(args: argparse.Namespace) -> List[str]:
    target_pages: List[str] = list(DEFAULT_TARGET_PAGES)
    target_pages.extend(args.target_page or [])

    if args.target_pages_file:
        path = Path(args.target_pages_file)
        with path.open("r", encoding="utf-8") as handle:
            for line in handle:
                cleaned = normalize_cli_title(line)
                if cleaned:
                    target_pages.append(cleaned)

    deduped: List[str] = []
    seen: Set[str] = set()
    for title in target_pages:
        cleaned = normalize_cli_title(title)
        key = cleaned.casefold()
        if cleaned and key not in seen:
            seen.add(key)
            deduped.append(cleaned)
...
def main() -> None:
    ...
    target_pages = load_target_pages(args)
